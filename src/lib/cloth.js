...
class Cloth {
  ...
  createGeometryFunction() {
    const { xSegments, ySegments, distance } =
      this;
    const stepX = xSegments * distance;
    const stepY = ySegments * distance;
    this.parametricGeometryFunc = function (
      u,
      v,
      vector
    ) {
    ...
    if (this.windEnabled) {
      geometry.faces.forEach((face) => {
        this.tmpForce
          ...
          .normalize()
          .multiplyScalar(
            face.normal.dot(this.windForce)
          );

        this.particles[face.a].addForce(
          this.tmpForce
        );
        this.particles[face.b].addForce(
          this.tmpForce
        );
        this.particles[face.c].addForce(
          this.tmpForce
        );
      });
    }
  ...
  satisfyConstraints() {
    this.constraints.forEach(
      ([p1, p2, distance]) => {
        ...
        const correction =
          this.diff.multiplyScalar(
            1 - distance / currentDist
          );
        const correctionHalf =
